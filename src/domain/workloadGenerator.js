...
function createKeySelector({ recordCount, requestDistribution, rng, theta }) {
  ...
  if (requestDistribution === "zipfian") {
    const keyGen = new ScrambledZipfianGenerator(
      0,
      recordCount - 1,
      theta,
      rng,
...
function createWorkload({
  ...
  if (!mix) {
    throw new Error(
      `Unknown workload "${workload}". Expected one of ${Object.keys(WORKLOADS).join(", ")}`,
    );
  }

  const rng = new XorShift128Plus(BigInt(seed));
  const selectKey = createKeySelector({
    recordCount,
    requestDistribution,
    rng,
    theta,
